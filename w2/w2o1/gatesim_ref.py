# (c) 2018 HvA.nl, f.h.schippers@hva.nl
# Python3.7 / Python2.7
...
class Bit:
    ...
    def __init__(self, bit=None, name=''):
        ...
        self.name = name
    def __str__(self):
        return '<Reg:{} bit={}>'.format(self.name, _str.get(self.bit))
    ...
    def get(self):
        if self.bit == None:
            raise Exception("Get:Uninitialised reg: {}".format(self.name))
        return self.bit

    def put(self, bit):
        if self.bit != None:
            raise Exception("Put: Initialised reg: {}".format(self.name))
        self.bit = bit
...
class Register:
    ...
    def __str__(self):
        return "<Reg:{} {}>".format(self._name,
                ''.join([ _str.get(r.peek()) for r in self._regs]))
...
class Not(Gate_11):
    def run(self):
        self.oc.put( self.ia.get() ^ 1 )

class And(Gate_21):
    def run(self):
        self.oc.put( ( self.ia.get() & self.ib.get() ) )

class Nand(Gate_21):
    def run(self):
        self.oc.put( ( self.ia.get() & self.ib.get() ^ 1 ) )
